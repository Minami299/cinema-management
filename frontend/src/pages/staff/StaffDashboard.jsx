...
export default function StaffDashboard({ initialTab = "overview" }) {
  ...
  const loadFoodOrders = async () => {
    try { setFoodOrders((await getFoodOrders()).data.data || []); }
    catch (error) { notify(error.response?.data?.message || "Không thể tải danh sách đơn Food & Drink."); }
  };
  ...
  useEffect(() => {
    ...
    if (initialTab === "food") { loadFoods(); loadFoodOrders(); }
  }, []);
